...
export default function NGSellersPage() {
  ...
  const [newSeller, setNewSeller] = useState({ name: '', code: '', reason: '' });
  ...
  async function addNGSeller() {
    if (!newSeller.name && !newSeller.code) {
      alert('出品者名または出品者コードを入力してください');
      return;
    }
  ...
  const exportToCSV = () => {
    try {
      ...
      const filename = `NG出品者_${timestamp}.csv`;
  ...
  const filteredSellers = ngSellers.filter(seller =>
    ...
    seller.code.includes(searchTerm)
  );
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <input
                ...
                onChange={(e) => setNewSeller({ ...newSeller, code: e.target.value })}
                className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <input
                type="text"
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">出品者コード</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredSellers.map((seller) => (
                      <tr key={seller.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">{seller.code || '-'}</td>
